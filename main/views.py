...
from main.models import user, files, folder, shared
...
def folder_click(request):
    if 'user_id' in request.session.keys() and request.session['user_id'] is not None:
        if request.GET and request.GET.__contains__('folder_id'):
            return render(request, 'folder_display.html', {'folder_id': request.GET['folder_id']})
        return render(request, 'folder_display.html')
    return redirect('index')
...
def login(request):
    ...
    if request.POST is not None and request.POST['email'] is not None and request.POST['password'] is not None:
        try:
            tempuser = user.objects.get(email_addr=request.POST['email'])
        ...
        if tempuser is None:
            ...
            if str(password) == str(tempuser.pswd):
                
                request.session['user_id'] = tempuser.id
                request.session['email_addr'] = tempuser.email_addr
                return 'dashboard'
...
def logout(request):
    ...
    del request.session['email_addr']
    return redirect('index')
...
def signup(request):
    ...
    if request.POST is not None and request.POST['email'] is not None and request.POST['password'] is not None:
        ...
        try:
            tempuser = user.objects.get(email_addr=mail)
        except user.DoesNotExist:
            tempuser = None
        ...
        tempuser = user(email_addr=mail, pswd=password)
        tempuser.save()
    return 'index'
...
def get_shared(request):
    ...
    for share_obj in obj.values():
        data_obj = {'id': share_obj['id'], 'email': user.objects.get(id=share_obj['sharedUser_id']).email_addr,
...
def check_share(request):
        obj = user.objects.get(email_addr=request.POST['share_email'])
        ...
        if shareRecord > 0:
            ...
            message = request.session['email_addr'] + " shared a <b>" + type + ".</b> \n checkout in the dashboard"
